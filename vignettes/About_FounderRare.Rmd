...
The corresponding code below, demonstrating a method to install the **FounderRare** package:
    ...
    devtools::install_github("username/FounderRare")  # Replace "username" with the actual GitHub username and adjust the path accordingly


Make sure to replace "**username**" in the **devtools::install_github()** function call with the correct GitHub username where the FounderRare package is hosted. Additionally, adjust the path if the package is not in the current working directory.

Please note that if FounderRare is not on GitHub or if the package structure is different, you may need to adjust the installation command accordingly.

Again I recommend to correctly loaded the dependencies libraries, here a code for that:

Ensure that the placeholder **username/** in the devtools::install_github() function invocation is substituted with the accurate GitHub username where the **FounderRare** package is located. It is important to note that if **FounderRare** is not hosted on GitHub or if the package structure deviates, it may be necessary to alter the installation command to suit the specific circumstances.
...
The dataset “IBDClusters” is an integral component of the ‘FounderRare’ package and serves as a representative example of the data required by the package in its input arguments. The command data("IBDClusters") is employed to load the dataset, while str(IBDClusters) provides a comprehensive overview of the dataset’s structure.
...
I always recommend to create the first Grange before tu use package function, because it give you chance to include some information not available in data like the number of chromosome, here the needed information to create the first element :
...
**Construction of a GRange for the Sliding Synthetic Gene (gr.Sliding_SG):**

The function **gr.Sliding_SG** is engineered to generate a Grange that represents a sliding synthetic gene (Sliding-SG) corresponding to a specific synthetic gene Grange (gr.SG). The provided code snippet exemplifies its application by invoking the function with parameters gr.SG and overlapSS = 10 (default value). Essentially, this function fabricates a sliding synthetic gene (SSG) with a length that is twice the specified overlap (overlapSS), given in kilobases (Kbp). The overlap parameter allows researchers to customize the window size.
...
The TBMD function is designed to transform the non-disjoint clusters present in the output list of SGs (SGList) from the TBM function into a refined output comprising segregated clusters. It sequentially examines each SG to determine if the IBD structure within the SG encompasses disjoint clusters, employing the **Disj_Clst_SG** function for this purpose. The function also offers an option to omit SG that lack any clusters in their IBD structure (remove=TRUE). The ultimate output of the **TBMD** function is a list of SG, each containing disjoint clusters within their IBD structures. The **Disj_Clst_SG** function applies principles of graph theory to convert the IBD structure into disjoint clusters. The input parameters encompass the SG of interest and IBD clusters. The function initiates by processing the input data, eliminating superfluous columns and managing missing values. Following this, it employs graph theory concepts to generate edges and construct a graph representation of haplotypes. The graph is subsequently partitioned into disjoint clusters. The function yields the modified SG with disjoint clusters in its IBD structure.


```{r }
# Generate a list of genomic segments using the TBM function
SGList.chr = TBM(IBDClusters, SGList, N=50)

# Use the TBMD function to segregate the clusters within each genomic segment
TBMD(SGList.chr, IBDClusters)

```

The R code example provided executes the TBM function with the parameters IBDClusters, SGList, and N=15 to generate a list of SGs (SGList.chr). This list is then passed to the TBMD function along with IBDClusters to segregate the clusters within each SG.
...
The R code provided first applies the **TBMD** function to the list of SG (SGList.chr) to segregate the clusters within each segment. Then, the **Smsg** function is used to compute the most shared SG for the segregated list (SGListD.chr). This is done twice, once without specifying the number of affected individuals, and once with Naff set to 19.

```{r }
# Segregate the clusters within each genomic segment
...
The subsequent lines of code are for saving the results. It is recommended to save the list of genomic segments in RDS format. The second line writes the computed Smsg data to a CSV file, providing a convenient and shareable format for further analysis and visualization. RDS files can be utilized to compute the IBD measure Sall, as proposed by Whittemore and Halpern (1994), for a single SG using the **Sall_SG** function or for all SGs of a chromosome using the **Sall** function. The inputs and output of Sall are analogous to those of Smsg. An example of Sall is not presented due to the memory and time constraints required by this function.

    # Save the list of genomic segments in RDS format
